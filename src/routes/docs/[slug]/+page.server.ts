...
Composable:
  ...
  Not(p)          Inverts match`
		...
		hookContract: {
			lang: 'json',
			code: `// stdin: Claude Code sends the tool call
{
  "tool_name": "Bash",
  "tool_input": {
    "command": "git commit -m \\"feat: add user endpoint\\""
  }
}

// stdout: klaudiush returns the decision
{
  "hookSpecificOutput": {
    "hookEventName": "PreToolUse",
    "permissionDecision": "allow"
		...
		hookEvents: {
			lang: 'text',
			code: `PreToolUse     Fires before a tool executes. Can block or allow.
PostToolUse    Fires after a tool completes. Observational only.
Notification   Fires on session events. Used for bell/dock bounce.`
		},
		errorCodeAnatomy: {
			lang: 'text',
			code: `  GIT 001
...
Prefixes:
  GIT    Git operation validators       (commit, push, branch, PR, ...)
  FILE   File format validators         (markdown, shellscript, terraform, ...)
  SEC    Secret detection               (API keys, tokens, private keys, ...)
  SHELL  Shell command validators       (backtick detection, ...)
  GH     GitHub-specific validators     (workflow pinning, ...)
  PLUG   Plugin system errors           (plugin crash, timeout, ...)
  SESS   Session tracking errors        (poisoned session, ...)`
		},
		sessionStates: {
			lang: 'text',
  ...
  +---------+    <--------------------    +----------+
       ^          SESS:<CODE> token             |
       |                                        | all commands
       |        session expired or              | denied with
       +------- new Claude session              | SESS001
                                                v`
		},
		exceptionEval: {
			lang: 'text',
			code: `1. Parse token from command (EXC:<CODE>:<REASON>)
   +-- Check env var KLACK first
   +-- Fall back to shell comment # EXC:...

2. Look up policy for <CODE>
   +-- No policy + require_explicit_policy -> deny
   +-- Policy found -> continue

3. Evaluate policy constraints
   +-- allow_exception = false -> deny
   +-- require_reason + no reason -> deny
   +-- min_reason_length not met -> deny
   +-- valid_reasons set + no match -> deny
   +-- All checks pass -> continue

4. Check rate limits
   +-- Per-code hourly/daily limit exceeded -> deny
   +-- Global hourly/daily limit exceeded -> deny

5. Log to audit trail, convert block to warning`
		},
		ruleEval: {
			lang: 'text',
			code: `1. Collect all enabled rules from global + project config
2. Sort by priority (highest first)
3. For each rule, check match conditions (AND logic):
   +-- validator_type matches current validator?
   +-- branch_pattern matches current branch?
   +-- file_pattern matches target file?
   +-- command_pattern matches command?
   +-- repo_pattern matches repo path?
4. First matching rule's action wins:
   +-- allow  -> skip all further validation (built-in + rules)
   +-- block  -> deny with rule's message
   +-- warn   -> add warning, continue validation`
		},
		pluginProtocol: {
			lang: 'text',
			code: `Discovery (--info flag):
  $ plugin --info
  {"name":"my-plugin","version":"1.0.0","description":"..."}

Validation (JSON over stdin/stdout):
  $ echo '{"tool_name":"Bash","command":"..."}' | plugin
  {"passed":true,"should_block":false}

  $ echo '{"tool_name":"Bash","command":"sudo rm -rf /"}' | plugin
  {"passed":false,"should_block":true,"message":"...","error_code":"..."}`
		},
		pluginLifecycle: {
			lang: 'text',
			code: `Dispatcher receives event
  |
  +-- Match plugin predicates against event
  |   (event_types, tool_types, command_patterns, file_patterns)
  |
  ...
  |
  +-- Merge plugin results with built-in validator results`
		},
		crashRecovery: {
			lang: 'text',
			code: `Exit codes:
  0   Normal operation (block/allow communicated via JSON)
  3   Crash (panic recovery triggered)

On panic:
  1. Recover from panic
  2. Write crash dump to ~/.klaudiush/crashes/
  3. Log error to stderr
  4. Exit 3 (Claude Code treats non-zero as "allow" - fail-open)

Crash dump contains:
  - Timestamp and klaudiush version
  - Stack trace
  - Sanitized hook event (secrets redacted)
  ...
  (built-in)  (~/.klaudiush/   (.klaudiush/     (KLAUDIUSH_*)  (--disable,
               config.toml)     config.toml)                    --config)`
		},
		backupFlow: {
			lang: 'text',
			code: `Config change detected
  |
  +-- Snapshot current config (full TOML dump)
  +-- Compute SHA-256 checksum
  +-- Store in ~/.klaudiush/.backups/{scope}/snapshots/
  +-- Update metadata.json with timestamp and hash
  |
  +-- Run retention policy
      +-- max_backups exceeded -> delete oldest
      +-- max_age exceeded -> delete expired
      +-- max_size exceeded -> delete oldest until under limit`
		}
	},
	migration: {
		jsonMigration: {
